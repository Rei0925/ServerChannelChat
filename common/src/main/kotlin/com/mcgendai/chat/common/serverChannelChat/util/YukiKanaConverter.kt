/**
 * LunaChatのYukiKanaConverterをKotlinにして流用。
 * @license    LGPLv3
 */
...
/**
 * 「ローマ字」から「かな文字」へ正確に変換するオブジェクト
 *
 * @author YukiLeafX,Rei0925
 * @see <a hrefv="https://support.microsoft.com/ja-jp/help/883232">https://support.microsoft.com/ja-jp/help/883232</a>
 */
object YukiKanaConverter {
    ...
    init {
        ...
        // 促音を追加する
        for (entry in builder.build().entries) {
            val romaji: String = entry.key!!
            val hiragana = entry.value
            if (canStartFromSokuon(romaji)) {
                builder.put(romaji.get(0).toString() + romaji, "っ$hiragana")
            }
        }
        ...
        ROMAJI_LIST = MAP.keys.toTypedArray<String?>()
        HIRAGANA_LIST = MAP.values.toTypedArray<String?>()
    }
    ...
    fun fixBrackets(text: String?): String? {
        val full: Array<String> = arrayOf("（", "）")
        val half: Array<String> = arrayOf("(", ")")
